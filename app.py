...
def analyze_naver_place(image_list):
    ...
    prompt = """이 이미지들은 네이버 플레이스(가게/업체) 관련 여러 페이지를 캡처한 것입니다.
이미지들은 같은 업체의 서로 다른 섹션(기본정보, 메뉴, 리뷰, 홈페이지, 블로그 등)일 수 있습니다.

중요: 
- 모든 이미지에서 같은 업체에 대한 정보를 찾아 통합하세요
- 각 이미지에서 유용한 정보를 최대한 추출하세요
- 중복되는 정보는 하나로 합쳐주세요
...
4. 메뉴/서비스/가격 정보가 있으면 포함해주세요
5. 리뷰에서 언급된 장점이 있으면 특징에 반영해주세요
6. 이미지에서 보이는 모든 텍스트를 추출해주세요
